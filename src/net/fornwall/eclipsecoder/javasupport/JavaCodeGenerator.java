...
public class JavaCodeGenerator extends CodeGenerator {
	...
	private String getCreateStatement(Object value, String variableName) {
		...
		if (type.isArray()) {
			StringBuilder builder = new StringBuilder(getTypeString(type) + " "
					+ variableName + " = new " + getTypeString(type) + "{");
	...
	public String getTestsSource() {
		...
		boolean doubleReturn = (multiDimensional ? problemStatement
				.getReturnType().getComponentType() : problemStatement
		...
		result.append("\npublic class "
				...
				+ "    protected " + problemStatement.getSolutionClassName()
				+ " solution;\n\n" + "    @Before\n"
				+ "    public void setUp() {\n" + "        solution = new "
		...
		if (doubleReturn) {
			result.append(multiDimensional ? COMPARE_DOUBLE_ARRAYS
					: COMPARE_DOUBLES);
		}

		int i = -1;
		for (ProblemStatement.TestCase testCase : problemStatement
				.getTestCases()) {
			i++;
			result.append("    @Test");
			...
			for (int param = 0; param < problemStatement.getParameterNames()
					.size(); param++) {
				result
						.append("        "
								+ getCreateStatement(
										testCase.parameters[param],
										problemStatement.getParameterNames()
												.get(param)));
			}

			result
					.append("\n        "
							+ getCreateStatement(testCase.getReturnValue(),
									"expected"));
